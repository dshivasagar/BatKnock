              ...
              <TouchableOpacity key={bat.id}
                ...
                <View style={{ flex: 1 }}>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <AppText style={{ color: theme.text, fontSize: fs(17), fontWeight: '800' }}>{bat.name}</AppText>
                    <AppText style={{ color: theme.accent, fontSize: fs(16), fontWeight: '800' }}>{bat.total_knocks || 0}</AppText>
                  </View>
                  <AppText style={{ color: theme.textSub, fontSize: fs(14), marginTop: 3 }}>{bat.brand}{bat.willow_type ? ` · ${bat.willow_type}` : ''}</AppText>
                  ...
                  <AppText style={{ color: theme.textSub, fontSize: fs(11), marginTop: 4 }}>
                    {Math.round(pct)}% · {bat.total_knocks || 0} / {bat.target_knocks || 10000} knocks
                  </AppText>
                </View>
                <AppText style={{ color: theme.textMuted, fontSize: fs(18), marginLeft: 8 }}>›</AppText>
              </TouchableOpacity>
            );
          })
        )}
